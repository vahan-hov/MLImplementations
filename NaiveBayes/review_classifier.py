...
class ReviewClassifier:
    def __init__(self, pos_folder, neg_folder, pos_test_folder, save_path, refresh=False):
    ...
    def prob_all_features_given_label(new_review_words, occurrences_pos, data_len):
        ...
        prob_being_in_review = preprocessing.normalize([prob_list])
        prob_being_in_review = reduce(mul, prob_being_in_review[0])
    ...
    def test(self):
        correct = 0
        new_reviews = os.listdir(self.pos_test_folder)
        for review in new_reviews:
            if self.predict(f'{self.pos_test_folder}/{review}') == 'pos':
                correct += 1
        print('Accuracy: ', correct / len(new_reviews))
